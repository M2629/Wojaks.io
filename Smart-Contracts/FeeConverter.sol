...
contract FeeConverter is Ownable, IFeeConverter {
   ...
   * The new version uses TWAP to pull CLOVER balance from the caller.
  ...
  */
  function convert(
    ...
    uint cloverAmount = cloverInput(supplyToken, inputAmount);
    IERC20(supplyToken).transfer(_originalCaller, inputAmount);
    cloverToken.transferFrom(_originalCaller, stakingPool, cloverAmount);
  ...
  function cloverInput(address _fromToken, uint _fromAmount) public view returns(uint) {

    uint priceFrom = lendingController.tokenPrice(_fromToken)         * 1e18 / 10 ** IERC20(_fromToken).decimals();
    uint priceTo   = lendingController.tokenPrice(address(cloverToken)) * 1e18 / 10 ** IERC20(address(cloverToken)).decimals();
